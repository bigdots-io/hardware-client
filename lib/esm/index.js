import { coordinates, createDisplayEngine, text, } from "@bigdots-io/display-engine";
...
let updateQueue = [];
if (!options.emulate) {
    matrix = new LedMatrix(Object.assign(Object.assign({}, LedMatrix.defaultMatrixOptions()), { rows: parseInt(options.rows, 10), cols: parseInt(options.cols, 10), parallel: parseInt(options.parallel, 10), multiplexing: parseInt(options.multiplexing, 10), chainLength: parseInt(options.chainLength, 10), hardwareMapping: GpioMapping.Regular }), Object.assign(Object.assign({}, LedMatrix.defaultRuntimeOptions()), { gpioSlowdown: 2 }));
